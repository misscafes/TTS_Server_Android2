...
- **四次调整（2026-07-27）**：
  1. 按用户最新要求，仅对 `🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组` 重新生成/修正 `previewText`；`呱呱` 仅处理重复或语句不佳的条目；`火山豆包`、`游音`、`Mossland` 保持原样不改动
  2. 生成规则：根据 `displayName` 关键词判断风格（游戏/古风/温柔/少年/老年/职业等），从对应 tone 词池中选取场景、动作、情绪、台词，组合为较长场景化独白；模板中不包含 `{displayName}`，确保预览文字不出现完整音色名称
  3. 对需修改分组的去重处理：重复条目追加唯一诗意结尾，避免与 `火山豆包`、`游音`、`Mossland` 的原有 `previewText` 冲突；仅修改指定分组，不影响保持原样的分组
...
- **验证**：
  ...
  - 全部 4133 条配置的 `previewText` 均已非空；`🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组`、`呱呱` 内无重复，`火山豆包`、`游音`、`Mossland` 保持原样（含其内部原有重复）
  - `🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组`、`呱呱` 中未出现完整 `displayName` 文本
  - `火山豆包`、`游音`、`Mossland` 的 `previewText` 与合并前源文件完全一致
...
- **已完成事项**：
  ...
  11. 按用户最新要求精细化 `previewText`：仅对 `🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组` 生成场景化独白；`呱呱` 仅处理重复/不佳条目；`火山豆包`、`游音`、`Mossland` 保持原样
  12. 生成规则：根据 `displayName` 风格关键词选择 tone 词池，模板中不含完整 `displayName`，产出较长、优美、非问候式独白；重复项追加唯一诗意结尾去重
  13. 验证：`🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组`、`呱呱` 内无重复，`displayName` 未出现在 previewText 中；`火山豆包`、`游音`、`Mossland` 与源文件一致
  14. 清理临时脚本及所有中间验证文件
- **涉及文件**：
  - `AGENTS.md`
  - `参考/jread_voice_六组排序.json`（重新生成）
