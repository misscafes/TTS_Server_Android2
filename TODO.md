...
#### 配置合并：六组排序 + Mossland → 七组排序
- **需求来源**：用户要求将 `jread_voice_Mossland_581..json` 作为第 7 个顶级分组合并到 `jread_voice_六组排序.json` 之后，`voiceTag` 从 `jread_voice_🪅薇薇的全部演员组_去重.json` 各子分组最大编号 +1 开始连续编号，并单独生成新文件。
- **涉及文件**：
  - `参考/jread_voice_六组排序.json`（源文件，保留）
  - `参考/jread_voice_Mossland_581..json`（源文件，保留）
  - `参考/jread_voice_🪅薇薇的全部演员组_去重.json`（用于计算起始 tag）
  - `参考/jread_voice_七组排序.json`（新增）
- **实现内容**：
  1. 读取 `六组排序` 6 个顶级分组（3552 条配置）
  2. 将 `Mossland` 从 `jread_voice_config_bundle` 转换为 `GroupWithSystemTts` 列表格式，仅做格式转换
  3. 保持 Mossland 原始子分组名称和顺序不变（女性儿童通用、女性少年通用、女性青年通用、女性中年通用、女性老年通用、男性儿童通用、男性少年通用、男性青年通用、男性中年通用、男性老年通用）
  4. 保持 Mossland 原始 `pluginId`、`locale`、`voice`、`data`、`enabled` 等字段不变
  5. 不强制设置 `speechRule` 和 `tagRuleId`
  6. 以 `薇薇的全部演员组_去重` 各 emoji 子分组映射后的标准子分组最大 `voiceTag` 编号 +1 作为 Mossland 对应子分组的起始编号
  7. 编号规则：当 tag 编号达到 1000 时，跳过 1000~1099，直接从 1100 开始继续编号
  8. 新父分组 `order` 设为 7，子分组 `order` 保持 Mossland 原始顺序 1~10
  9. 新分组 ID 从现有最大父分组 ID + 1000000000 起分配，子分组 ID 按 `父分组 ID + 1000 + order` 分配
  10. 单独提取第 7 分组 Mossland 为独立文件 `参考/jread_voice_Mossland_第7分组.json`
- **二次调整（2026-07-27）**：
  1. 统一 `七组排序` 中所有二级分组名称：`🦋女童`→`女性儿童通用`、`🦋少女`→`女性少年通用`、`🦋女青年`→`女性青年通用`、`🦋女中年`→`女性中年通用`、`🦋女老年`→`女性老年通用`、`🦋男童`→`男性儿童通用`、`🦋少年`→`男性少年通用`、`🦋男青年`→`男性青年通用`、`🦋男中年`→`男性中年通用`、`🦋男老年`→`男性老年通用`
  2. 统一二级分组排序：每个父分组下均按 `女性儿童通用 → 女性少年通用 → 女性青年通用 → 女性中年通用 → 女性老年通用 → 男性儿童通用 → 男性少年通用 → 男性青年通用 → 男性中年通用 → 男性老年通用` 排列
  3. 同步更新 `参考/jread_voice_Mossland_第7分组.json` 的子分组顺序
- **三次调整（2026-07-27）**：
  1. 为 `七组排序` 中所有空 `previewText` 填充语音测试文本：
     - 第 7 分组 Mossland：直接拷贝 `data.mosslandReferenceText` 到 `source.previewText`
     - 前 6 个分组：根据 `displayName` 和二级分组（年龄/性别）自动生成一段较长的场景化独白
  2. 生成规则：使用多种中文场景、动作、情绪、台词模板组合，确保不是简单问候
  3. 对填充后的所有 `previewText` 进行去重处理，确保 4133 条配置全部不同
- **四次调整（2026-07-27）**：
  1. 按用户最新要求，仅对 `🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组` 重新生成 `previewText`；`火山豆包`、`游音`、`Mossland` 保持原样不改动；`呱呱` 从 `jread_voice_五组排序.json` 原样恢复原有 `previewText`，不做任何修改或去重
  2. 生成规则：按二级分组（性别/年龄）确定默认气质，再根据 `displayName` 关键词（游戏/古风/温柔/高冷/御姐/病娇/少年/老年/职业等）选择对应 tone；每个 tone 包含多套 `opening` + `ending`，组合为 1~3 句话、约 50 字的通顺独白；模板中不出现完整 `displayName`
  3. 对 `🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组` 做全局去重：新生成的文本不与 `火山豆包`、`游音`、`Mossland`、`呱呱` 的原有 `previewText` 重复，也不与自身已生成文本重复；极个别无法直接生成唯一文本时，追加诗意短句作为后缀
  4. 重新提取 `参考/jread_voice_Mossland_第7分组.json`，确保与 `七组排序` 中的 Mossland 数据一致
  5. 清理本次使用的临时脚本（`rebuild_seven.py`、`final_dedup.py`、`regen_preview.py`、`regen_preview_v3.py`、`fix_preview_v2.py`）及所有中间验证/检查文本文件
- **验证**：
  - 合并后顶级分组数：7
  - 合并后配置总数：4133（3552 + 581）
  - 第 7 分组 Mossland 条目数：581
  - Mossland 各子分组 `voiceTag` 已从去重后最大编号 +1 连续编号，且 1000~1099 区间无 tag（女性青年通用 943~999、1100~1219）
  - 全部 4133 条配置的 `previewText` 均已非空
  - `🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组` 内部无重复，且与 `火山豆包`、`游音`、`Mossland`、`呱呱` 无重复
  - 重新生成的 `previewText` 字数集中在 40~58 字，平均约 45 字，符合 1~3 句话、50 字左右的要求
  - `呱呱` 保持原有 `previewText`，恢复自 `jread_voice_五组排序.json`，内部原有重复保留未改
  - `Mossland` 保持原样，内部原有重复保留未改
  - `🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组`、`呱呱` 中未出现完整 `displayName` 文本
  - `火山豆包`、`游音`、`Mossland`、`呱呱` 的 `previewText` 与合并前源文件完全一致
  - 独立文件 `参考/jread_voice_Mossland_第7分组.json` 仅包含 Mossland 父分组及 10 个子分组，共 581 条配置
  - 已确认 `七组排序` 中不存在非标准二级分组名称

#### 配置合并：五组排序 + 薇薇全部演员组 → 六组排序
- **需求来源**：用户要求将 `jread_voice_🪅薇薇的全部演员组_去重.json` 直接合并到 `jread_voice_五组排序.json` 之后，形成 6 个顶级分组的快导配置列表，且不对全部演员组做任何额外修改。
- **涉及文件**：
  - `参考/jread_voice_五组排序.json`（源文件，保留）
  - `参考/jread_voice_🪅薇薇的全部演员组_去重.json`（源文件，保留）
  - `参考/jread_voice_六组排序.json`（重新生成）
- **实现内容**：
  1. 读取 `五组排序` 5 个顶级分组（2603 条配置）
  2. 将 `🪅薇薇的全部演员组_去重` 从 `jread_voice_config_bundle` 转换为 `GroupWithSystemTts` 列表格式，仅做格式转换
  3. **不修改**全部演员组的子分组名称，保留原始 `🦋女青年`、`🦋男青年` 等 10 个 emoji 子分组
  4. **不修改**全部演员组的 `voiceTag`、`pluginId`、`data` 等字段
  5. **不强制设置** `speechRule` 和 `tagRuleId`（因全部演员组与五组排序不是同一底层插件/朗读规则）
  6. 新父分组 `order` 设为 6，子分组 `order` 保持 bundle 原始顺序 1~10
  7. 新分组 ID 从现有最大父分组 ID + 1000000000 起分配，子分组 ID 按 `父分组 ID + 1000 + order` 分配，避免与现有分组冲突
- **验证**：
  - 合并后顶级分组数：6
  - 合并后配置总数：3552
  - 各顶级分组条目数：薇薇通用 233、火山豆包 98、2次元 731、呱呱 960、游音 581、🪅薇薇的全部演员组 949

### 项目规范更新（2026-07-27）

#### 规范新增：全程中文输出
- **需求来源**：用户要求明确项目规范，确保所有与本项目相关的工作输出均使用中文。
- **涉及文件**：
  - `AGENTS.md`（已更新）
- **实现内容**：
  1. 在 `AGENTS.md` 的「进入项目必读」中新增条款：所有项目工作输出（回复、代码注释、提交信息、文档及内部思考/分析）必须全程使用中文。
- **验证**：
  - `AGENTS.md` 已包含全程中文输出条款

### 参考目录配置更新（2026-07-26）

##### 参考目录调整：删除猫箱VV所有生成文件，恢复到五组排序状态
- **需求来源**：用户要求删除基于 `参考/猫箱VV合成核心（356+547预设）-PCM前台暖场流式v2-厂家Key.json` 生成的所有快导配置文件，恢复到仅保留 `jread_voice_五组排序.json` 的状态
- **涉及文件**：
  - `参考/jread_voice_薇薇通用.json`（已删除）
  - `参考/jread_voice_薇薇性格无通用.json`（已删除）
  - `参考/generate_maoxiang_vv_quick_import.py`（已删除）
  - `参考/jread_voice_猫箱VV_快导配置列表.json`（已删除）
- **实现内容**：
  1. 删除 `jread_voice_薇薇通用.json`、`jread_voice_薇薇性格无通用.json`
  2. 删除 `generate_maoxiang_vv_quick_import.py`
  3. 删除 `jread_voice_猫箱VV_快导配置列表.json`
  4. 仅保留猫箱VV源插件包文件 `参考/猫箱VV合成核心（356+547预设）-PCM前台暖场流式v2-厂家Key.json`
- **验证**：
  - 参考目录下已无猫箱VV快导配置生成文件（`jread_voice_猫箱VV_快导配置列表.json`、`jread_voice_薇薇通用.json`、`jread_voice_薇薇性格无通用.json`、`generate_maoxiang_vv_quick_import.py`）
  - `jread_voice_五组排序.json` 保持原状

#### 参考目录配置处理：`🪅薇薇的全部演员组` 去重并重新编号
- **需求来源**：用户要求对比 `参考/jread_voice_🪅薇薇的全部演员组_1176.json` 与 `参考/新分组/jread_voice_薇薇通用_233.json`，删除重复内容后生成新的全演员组，并按五组排序最后的 tag 顺序往后递增
- **涉及文件**：
  - `参考/jread_voice_🪅薇薇的全部演员组_1176.json`（源文件，保留）
  - `参考/新分组/jread_voice_薇薇通用_233.json`（对比文件）
  - `参考/jread_voice_🪅薇薇的全部演员组_去重.json`（新增）
- **实现内容**：
  1. 读取全演员组 1176 条 configs 和薇薇通用 233 条 configs
  2. 以 `data.contextTexts` 作为重复判定依据，找出并删除全演员组中与薇薇通用 233 重复的 227 条
  3. 将全演员组的 10 个子分组映射到标准子分组：
     - `🦋女童` → `女性儿童通用`、`🦋男童` → `男性儿童通用`
     - `🦋少女` → `女性少年通用`、`🦋少年` → `男性少年通用`
     - `🦋女青年` → `女性青年通用`、`🦋男青年` → `男性青年通用`
     - `🦋女中年` → `女性中年通用`、`🦋男中年` → `男性中年通用`
     - `🦋女老年` → `女性老年通用`、`🦋男老年` → `男性老年通用`
  4. 按五组排序同子分组最后一个 tag 编号往后递增，为剩余 949 条重新分配 `voiceTag`
- **验证**：
  - 去重后 configs：949 条
  - 与 `薇薇通用_233` 无 `contextTexts` 重复
  - 各子分组 tag 编号均正确接续五组排序最后一个编号
  - 10 个分组结构保持不变
...
- **需求来源**：批量分配标签时，若选中项 `tagRuleId` 不一致或对应朗读规则无标签，对话框提示"朗读规则不一致"或"无可用的标签"，无法强制分配
- **涉及文件**：
  - `app/src/main/java/com/github/jing332/tts_server_android/compose/systts/list/BatchTagDialog.kt`
  - `app/src/main/res/values/strings.xml`
- **实现内容**：
  1. 移除"朗读规则不一致"阻断：无论选中项 `tagRuleId` 是否一致，都允许批量分配标签
  2. `effectiveTags` 合并数据库朗读规则标签与所有选中项已有的 `tag` / `tagName` 集合
  3. 当存在现成标签时，使用下拉框选择起始标签并按顺序分配
  4. 当没有任何现成标签时，提供两个输入框让用户自由输入 `标签` 和 `标签名`，强制分配
  5. `tagRuleId` 不一致时保持各条目原有 `tagRuleId`；一致时统一更新为公共 `tagRuleId`
  6. 新增字符串资源 `tag_name` 用于自由输入框标签
- **验证**：
  - `./gradlew :app:compileAppDebugKotlin` 编译通过
  - `./gradlew :app:assembleAppRelease` 构建成功，生成 `newapk/TTS-Server-v1.26.071912-3113.apk` 和 `newapk/TTS-Server-latest.apk`
...
#### 资源更新：TTSOnline官方音色分组策略调整
- **需求来源**：用户需要 `TTSOnline官方` 在完整版配置中保持简洁的性别分组，同时单独提供一个按 `tags` 细分的导入文件
- **涉及文件**：
  - `参考/jread_voice_千问全家桶2852_四大组_快导配置列表.json`
  - `参考/jread_voice_TTSOnline官方_分类导入版.json`
- **实现内容**：
  1. 完整版中 `TTSOnline官方` 保持按性别/年龄分为 `女性青年通用` / `男性青年通用` 两个子分组（共 433 条音色）
  2. 单独提取文件 `参考/jread_voice_TTSOnline官方_分类导入版.json` 中 `TTSOnline官方` 按 `tags` 字段细分为 17 个分类子分组
  3. 17 个分类：openai、上新、二次元、动画鬼畜、多情感、广告解说、微软、推荐、方言、旧版、有声书、模仿、热门、女生、男生、童声、通用
  4. 单独提取文件中同一音色若属于多个 tag，会分别出现在对应分类下（共 1178 条配置项）
- **生成结果**：
  - `参考/jread_voice_千问全家桶2852_四大组_快导配置列表.json`（59 个分组，2852 条配置项）
  - `参考/jread_voice_TTSOnline官方_分类导入版.json`（18 个分组，1178 条配置项）
...
### 2026-07-27 本次会话（更新项目规范 + 合并六组排序 + 精细化 previewText）
- **当前版本**：v1.26.072513（基于 `hhh4` 分支）
- **已完成事项**：
  1. 根据用户要求，在 `AGENTS.md` 的「进入项目必读」中新增全程中文输出条款
  2. 将 `参考/jread_voice_五组排序.json` 与 `参考/jread_voice_🪅薇薇的全部演员组_去重.json` 合并
  3. 生成新文件 `参考/jread_voice_六组排序.json`，包含 6 个顶级分组、共 3552 条配置
  4. 按用户要求重新生成：全部演员组不做任何标准化处理，保留原始子分组名称、voiceTag、pluginId 和 data，不强制设置 speechRule/tagRuleId
  5. 将 `参考/jread_voice_Mossland_581..json` 作为第 7 个顶级分组合并到 `六组排序` 之后
  6. 生成新文件 `参考/jread_voice_七组排序.json`，包含 7 个顶级分组、共 4133 条配置
  7. Mossland 的 `voiceTag` 从 `薇薇的全部演员组_去重` 各子分组最大编号 +1 开始连续编号
  8. 编号规则：达到 999 后跳过 1000~1099，从 1100 继续编号
  9. 单独提取第 7 分组 Mossland 为 `参考/jread_voice_Mossland_第7分组.json`
  10. 统一 `七组排序` 所有二级分组名称为标准名称，并按 `女性儿童通用…女性老年通用、男性儿童通用…男性老年通用` 排序
  11. 按用户最新要求重新生成 `previewText`：仅对 `🪅薇薇通用`、`2次元`、`🪅薇薇的全部演员组` 生成 1~3 句话、约 50 字的通顺独白；`火山豆包`、`游音`、`Mossland` 保持原样；`呱呱` 从 `五组排序` 原样恢复原有 `previewText`，不修改不去重
  12. 生成规则：按性别/年龄确定默认气质，按 `displayName` 关键词（游戏/古风/温柔/高冷/御姐/病娇/少年/老年/职业等）选择 tone；每个 tone 提供多套 opening/ending 组合；模板中不出现完整 `displayName`
  13. 去重：修改分组内部无重复，且不与 `火山豆包`、`游音`、`Mossland`、`呱呱` 的原有文本重复
  14. 验证：修改分组字数集中在 40~58 字，平均约 45 字；`displayName` 未完整出现在 previewText 中；`火山豆包`、`游音`、`Mossland`、`呱呱` 与源文件一致
  15. 清理临时脚本及所有中间验证文件
- **涉及文件**：
  - `AGENTS.md`
  - `参考/jread_voice_六组排序.json`（重新生成）
  - `参考/jread_voice_七组排序.json`（新增/更新）
  - `参考/jread_voice_Mossland_第7分组.json`（新增/更新）
  - `TODO.md`（本记录）
- **注意事项**：
  - 后续所有项目相关输出（回复、注释、文档、内部思考）均使用中文
  - 新合并文件格式为 `GroupWithSystemTts` 列表，可直接作为普通 TTS 配置列表导入
  - 全部演员组和 Mossland 均保持原样，仅做格式转换和追加；后续按用户要求统一了二级分组名称与顺序
  - `previewText` 全部非空；修改分组内无重复；保持原样的分组内部原有重复未改动

### 2026-07-26 本次会话（参考/删除猫箱VV生成文件 + 薇薇全部演员组去重编号）
- **当前版本**：v1.26.072513（基于 `hhh4` 分支）
- **已完成事项**：
  1. 删除所有基于猫箱VV插件生成的快导配置文件：`参考/jread_voice_猫箱VV_快导配置列表.json`、`参考/jread_voice_薇薇通用.json`、`参考/jread_voice_薇薇性格无通用.json`
  2. 删除生成脚本 `参考/generate_maoxiang_vv_quick_import.py`
  3. 恢复到仅保留 `jread_voice_五组排序.json` 的状态
  4. 对比 `参考/jread_voice_🪅薇薇的全部演员组_1176.json` 与 `参考/新分组/jread_voice_薇薇通用_233.json`
  5. 以 `data.contextTexts` 为重复判定依据，删除全演员组中重复的 227 条 configs
  6. 将全演员组 10 个子分组映射到标准子分组，按五组排序最后一个 tag 编号往后递增，为剩余 949 条重新分配 `voiceTag`
  7. 生成新文件 `参考/jread_voice_🪅薇薇的全部演员组_去重.json`
  8. 验证：去重后无重复、tag 编号均正确接续五组排序
- **涉及文件**：
  - `参考/jread_voice_猫箱VV_快导配置列表.json`（已删除）
  - `参考/jread_voice_薇薇通用.json`（已删除）
  - `参考/jread_voice_薇薇性格无通用.json`（已删除）
  ...
  - `TODO.md`（本记录）
- **注意事项**：
  - 当前参考目录下仅保留猫箱VV源插件包 `参考/猫箱VV合成核心（356+547预设）-PCM前台暖场流式v2-厂家Key.json`
  - `jread_voice_五组排序.json` 保持原状
  - 全演员组去重文件为 `jread_voice_config_bundle` 格式，导入方式与源文件一致
...
- **已完成事项**：
  1. 删除旧的 `参考/jread_voice_五组排序.json` 及所有临时/备份文件
  2. 从 `参考/分组拆分/` 重新读取五组源文件：
     - `jread_voice_薇薇通用_233.json`
     - `jread_voice_千问全家桶2852_火山豆包.json`
     - `jread_voice_千问全家桶2852_2次元.json`
     - `jread_voice_千问全家桶2852_呱呱.json`
     - `jread_voice_千问全家桶2852_游音.json`
  3. 按用户指定顺序合并：🪅薇薇通用 → 火山豆包 → 2次元 → 呱呱 → 游音
  4. 父分组 `order` 重设为 1~5
  5. 每个父分组下的子分组 `order` 从 1 开始连续重排
  6. 音色标签（`tagName`/`voiceTag`/`speechRule.tag`）按**子分组类型跨父分组**连续编号，从 1 开始
  7. 生成新文件：`参考/jread_voice_五组排序.json`
  8. 同步修改 `参考/分组拆分/` 下 5 个源文件：父分组 `order`、子分组 `order`、音色标签编号与合并后文件保持一致
- **涉及文件**：
  - `参考/jread_voice_五组排序.json`
  - `参考/分组拆分/jread_voice_薇薇通用_233.json`
  - `参考/分组拆分/jread_voice_千问全家桶2852_火山豆包.json`
  - `参考/分组拆分/jread_voice_千问全家桶2852_2次元.json`
  - `参考/分组拆分/jread_voice_千问全家桶2852_呱呱.json`
  - `参考/分组拆分/jread_voice_千问全家桶2852_游音.json`
- **注意事项**：
  - 当前标签编号规则为「按子分组类型跨父分组连续编号」。例如 `女性少年通用`：薇薇 1-20 → 豆包 21-37 → 2次元 38-116 → 呱呱 117-137 → 游音 138-241
  - 若后续需要改为「每个父分组内部独立编号」或其他规则，需单独说明
  - 临时验证文件已清理
  - `git push origin hhh4` 成功；`git push github hhh4` 因网络连接失败（Connection reset），可稍后手动重试
- **后续可复用流程（五组排序）**：
  1. 清理旧 `参考/jread_voice_五组排序.json` 及临时文件
  2. 从 `参考/分组拆分/` 读取五组源文件
  3. 按「薇薇 → 豆包 → 2次元 → 呱呱 → 游音」顺序合并
  4. 父分组 `order` 设为 1~5
  5. 每个父分组下子分组 `order` 从 1 开始连续
  6. 音色标签按子分组类型跨父分组连续编号（`tagName`/`voiceTag`/`speechRule.tag` 同步更新）
  7. 保存为 `参考/jread_voice_五组排序.json`
...
### 2026-07-19 本次会话（生成五组新排序合并配置）
- **当前版本**：v1.26.071915（基于 `hhh4` 分支）
- **已完成事项**：
  1. 读取 `参考/分组拆分/` 下的拆分文件：`薇薇通用`、`火山豆包`、`呱呱`、`游音`
  2. 将 `参考/新分组/jread_voice_2次元_731.json` 从 JRead bundle 格式转换为与其他拆分文件一致的 `GroupWithSystemTts` 列表格式
  3. 按用户指定顺序合并为新的快导配置列表：
     - 1. 🪅薇薇通用
     - 2. 火山豆包
     - 3. 2次元（使用新的 731 条版本）
     - 4. 呱呱
...
- **注意事项**：
  - 新的 `2次元` 共 731 条，比原拆分文件多 81 条，分组结构与条数分布均有变化
  - 合并时保留了各拆分文件原有的 `groupId`/`parentGroupId`，顶级组 ID 互不冲突
  - 未包含 `参考/分组拆分/` 中的 `鹿`、`官方音色`、`TTSOnline官方`
  - 如需调整文件名或增删分组，可在此基础上继续修改
